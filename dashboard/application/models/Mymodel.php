...
class Mymodel extends CI_Model {
	...
	{
	    $this->db->select('category.ID_CATEGORY_DB, category.NAME_CATEGORY, category.CREATED, case when category.UPDATED_BY is "0" then "-" else (select FULL_NAME from users where ID_USER = category.CREATED_BY) end "NamaAC" , category.UPDATED, (select FULL_NAME from users where ID_USER = category.UPDATED_BY) "NamaAU",  category.PUBLISHED');
